  ...
  const Footer: React.FC = () => {
    ...
    const handlesubscribe=async(e:React.FormEvent)=>{
      ...
      if(!email||!email.includes("@")){
        setmessage("Enter Invalid email")
        return
      }
      try{
      const res=await axios.post("https://cricket-backend-sigma.vercel.app/api/subscribe",{email})
      setmessage(res.data.message||"Subscribed successfully")
      setemail("")
    }
    catch(error){
      const err=error as AxiosError<{message:string}>
      const errmsg=err.response?.data?.message || err.message || "An error occured while subscribing"
